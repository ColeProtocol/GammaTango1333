...
export default function InProcessing() {
  ...
  const checkTasks = async (results, user) => {
    ...
    for(var i = 0; i < results.length; i++){
      ...
      await Firebase.app().firestore().collection("users").doc(user).collection('intasks').doc(strTask).get().then((docSnapshot) => {
        if (docSnapshot.exists) {
          ...
          Firebase.app().firestore().collection("users").doc(user).collection('intasks').doc(strTask).set({
            status: false,
          });
          let elem =
          {
  ...
  const getTasks = async () => {
    try {
      ...
      const request = await axios.get("http://3.19.67.76:1337/in-processings", {
        headers: {
  ...
  const handlePress = (cStatus, task, email) => {
    ...
    console.log(cStatus);
    var nStatus = !cStatus;
    Firebase.app().firestore().collection("users").doc(email).collection('intasks').doc(task).set({
      status: nStatus,
    });
    getTasks();
  };
